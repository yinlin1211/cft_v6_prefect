"""
CFT v3 训练脚本 — 评估代码修正版（最佳模型选择：val loss 最小）
==============================
...
在 v2 基础上的修正（v3）：
  ...
  [变体] 最佳模型选择标准：val loss 最小（不依赖后处理閘値）
...
def main():
    ...
    # 按启动时间生成独立 run 目录，格式：run/<时间戳>_val/
    # 每次启动都会创建全新子目录，多个进程并发训练时互不干扰
    run_timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_name = f"{run_timestamp}_val"
    ...
    pid_file = Path(f'/tmp/cft_v6_val_{run_timestamp}.pid')
