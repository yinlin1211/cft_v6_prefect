...
[train_COn_01.py 修改]
  - 阈值搜索精度改为 0.1 步长（onset/frame 均为 0.1~0.9，共 9×9=81 组合）
...
def find_best_threshold(model, val_dataset, criterion, device, hop_length, sample_rate,
    ...
    onset_thresholds = [round(x * 0.1, 1) for x in range(1, 10)]  # 0.1, 0.2, ..., 0.9
    frame_thresholds = [round(x * 0.1, 1) for x in range(1, 10)]  # 0.1, 0.2, ..., 0.9
